...
class MobileyeSub(Node):
    def __init__(self):
        super().__init__('dbw_mobileye_node')
        self.mobPub = self.create_publisher(MobileyeInfo, '/Mobileye_Info', 20)
        self.frame_ok = 0
        self.mobmsg = MobileyeInfo()
        self.obstacle_data = [ObstacleData() for _ in range(13)]
        self.next_lane = [LKAlane() for _ in range(8)]
        self.tsr = [TSR() for _ in range(7)]
        self.tsr_num = 0
        ...
        self.get_logger().info('Mobileye Subscriber Node Initialized')
    ...
    def data_parser(self, data):
        ...
        if data.id == 0x766:
            ...
            self.mobmsg.system_warnings.tsr_warning_level = self.frame_data[6] % 8  # Parece un nivel, así que no lo cambiamos a bool
            self.mobmsg.system_warnings.headway_warning_level = self.frame_data[7] % 4  # Igual, es un nivel, no bool
            ...
            self.mobmsg.car_info.speed = self.frame_data[2]  # Mantiene como entero porque representa velocidad
            self.mobmsg.car_info.shield_plus_settings = self.frame_data[7]  # Parece configuración, no se cambia a bool
